...
%% init    
  case 'init'
    ...
    ToggleParam(obj, 'reward_rate', 0, x, y, 'position', [x+200 y 200 20], ...
        'OffString', 'Rewarded trials are only total rate', ...
        'OnString',  'Rewarded trials can change rate', ...
        'TooltipString', 'Set the reward to include all trials or only the endpoints');    
    set_callback(reward_style, {mfilename, 'reward_rate'});
    next_row(y);
...
%                                             '\nIf light and sound, both.']))
    ...
    % *********************************************************************
    DispParam(obj, 'probe_rate', 12, x,y, 'position', [x y 100 20], ...
        'labelfraction', 0.5, ...
        'TooltipString', 'the underlying click rate for probe trials');
	...
	PushbuttonParam(obj, 'rate_resample', x, y, 'position', [x+300 y 100 20], ...
		'label', 'Resample', ...
		'TooltipString', 'resample rate, the mean underlying click rate');
	set_callback({rate_min; rate_max; rate_resample;}, {mfilename, 'rate_resample'});
    ...
    ToggleParam(obj, 'probe_type',  0, x, y, 'position', [x+500 y 100 20], ...
                            'OffString', 'alpha', ...
                            'OnString', 'rate',...
                            'TooltipString', 'what type of probe is indicated by probe_frac--regularity (alpha, 1) or rate (underlying click rate, 2)');
    next_row(y);
    ...
    feval(mfilename, obj, 'reward_rate');
...
%% make_sounds
  case 'make_this_sound'
    ...
    if (value(probe_type) == 1 && value(is_probe)) || (value(reward_rate))
        feval(mfilename, obj, 'rate_resample');
        this_trial_rate = value(probe_rate);
        total_rate.value = value(probe_rate);
        if (this_trial_rate * value(T) < 3)
            T.value = value(T_max); % if the T is too short for this trial, take the max T
        end
    else
        this_trial_rate = value(total_rate);
    end

    [snd,bpt] = make_time_train(this_trial_rate, value(ThisAlpha), value(srate), value(T), 'bup_width',value(bup_width), ...
...
%% next_trial
  case 'next_trial'  
      ...
      if value(reward_rate == 1) & value(probe_type == 1) 
          probe_frac.value = 0;
      end;
      if value(probe_frac) > rand
            is_probe.value = 1;
      else
            is_probe.value = 0;
      end
      if value(reward_style) && value(is_probe) && value(probe_type) == 0
        temp_side = side;
        side = 'p';
      end

...
%% init    
  case 'init'
    ...
    SoloParamHandle(obj, 'pbup_fig', ...
        'value', double(figure('Position', [200 50 1370 580], ...
	...
	NumeditParam(obj, 'sduration', 5, x, y, 'position', [x+300 y 100 20], ...
		'labelfraction', 0.7, ...
   ...
   % STIMULATOR SECTION
    ...
    next_row(y);    
    ToggleParam(obj, 'ao_is_sine_not_square', 0, x, y, 'position', [x+300 y 150 20], ...
		'OffString', 'Square wave', ...
		'OnString',  'Sine wave', ...
		'TooltipString', 'Sine or square wave?');
    ToggleParam(obj, 'stim_is_analog_not_digital', 0, x, y, 'position', [x+200 y 100 20], ...
    ...
    tooltip_ao_setup = ['To set up for analog control of laser power, read the instructions at ', ...
                        'https://brodylabwiki.princeton.edu/wiki/images/9/9b/Breakout_box_instructions.pdf ', ...
                        'for assigning the RJ-45 jack for analog output. Then, specify in ', ...
                        'the rig''s custom settings file the analog output line (1 or 2) and the clock ', ...
                        'speed by adding the lines "AO_LINES; LASER; <1 or 2>" and ', ...
                        'GENERAL; CLOCK SPEED; <a value up to 2000>'];
    DispParam(obj, 'is_set_up_for_analog_output', 0, x, y, 'position', [x y 190 20], ...
                   'labelfraction', 0.9,...
                   'label', 'Is the rig set up for analog output?', ...
                   'TooltipString', tooltip_ao_setup)
    next_row(y,2);
    ...
    next_row(y, 1.3);    
    % DISTRACTOR SECTION
    ToggleParam(obj, 'fix_nodiffs', 1, x, y, 'position', [x y 100 20], ...
                'OffString', 'No 0 diff resamp', ...
                'OnString',  '0 diff resamp ', ...
                'TooltipString', 'With lower click rates, harder trials are more likely to have no click difference. Remake the sound if on and a sound has no click diff');
    next_row(y);
    ToggleParam(obj, 'timing_dist', 0, x, y, 'position', [x y 100 20], ...
                'OffString', 'No stim time distractor', ...
                'OnString',  'Stim Time distractor ', ...
                'TooltipString', 'If on, stimulus timing statistics will be distractor');
    NumeditParam(obj, 'timing_dist_percent',0, x, y, 'position', [x+100 y 250 20], ...
                'labelfraction', 0.6, ...
                'TooltipString', 'Percentage of total trials that will be timing distractor trials');
    MenuParam(obj, 'timing_dist_type', {'Fixed', 'Poisson'}, 1, x, y, 'position', [x+350 y 250 20], ...
                    'labelfraction', 0.4, ...
                    'TooltipString', 'Type of distribution for timing distractor, should be opposite of "fixed_fraction"');
    next_row(y);
    ToggleParam(obj, 'freq_dist', 0, x, y, 'position', [x y 100 20], ...
                'OffString', 'No click frequency distractor', ...
                'OnString',  'Click frequency distractor ', ...
                'TooltipString', 'If on, stimulus frequncy will be distractor');
    NumeditParam(obj, 'freq_dist_percent', 0, x, y, 'position', [x+100 y 250 20], ...
                'labelfraction',0.6, ...
                'TooltipString', 'Percentage of total trials that will be frequency distractor trials');
    NumeditParam(obj, 'freq_dist_Hz', 2500, x, y, 'position', [x+350 y 250 20], ...
                    'labelfraction',0.4, ...
                    'TooltipString', 'Frequency of distractor clicks in Hz, will be used as tones input to make_pbup, can be a vector');
    next_row(y);
    ToggleParam(obj,'location_dist',0,x,y,'position',[x y 100 20],...
                'OffString','No location distractor', ...
                'OnString','Location distractor', ...
                'TooltipString','If on, stimulus location (L/R) will be distractor');
    NumeditParam(obj, 'location_dist_percent', 0, x, y, 'position', [x+100 y 250 20], ...
                'labelfraction',0.6, ...
                'TooltipString', 'Percentage of total trials that will be location distractor trials');
    MenuParam(obj, 'location_dist_side', {'Left', 'Right','Both','Rand'}, 1, x, y, 'position', [x+350 y 250 20], ...
                'labelfraction', 0.4, ...
                'TooltipString', 'Side of distractor clicks, if both, then will be played on both sides, if rand, then randomly chosen');
    next_row(y);
    DispParam(obj, 'is_time_dist', 0, x, y, 'position', [x y 100 20], ...
        'labelfraction', 0.8, ...
        'TooltipString', 'whether this trial is a timing distractor trial (based on timing_dist_percent)');
    DispParam(obj, 'is_freq_dist', 0, x, y, 'position', [x+100 y 100 20], ...
        'labelfraction', 0.6, ...
        'TooltipString', 'whether this trial is a frequency distractor trial (based on freq_dist_percent)');
    DispParam(obj, 'is_loc_dist', 0, x, y, 'position', [x+200 y 100 20], ...
        'labelfraction', 0.6, ...
        'TooltipString', 'whether this trial is a location distractor trial (based on location_dist_percent)');
    next_row(y);
    SubheaderParam(obj, 'title5', 'Distractor Section', x, y);
    ...
    next_row(y);
	NumeditParam(obj, 'T_probe', x, y, 10, 'position', [x y 150 20], ...
		'TooltipString', 'in addition to the Ts specified by T_min and T_max, T can be this value with probability p_probe');
    ...
    ToggleParam(obj, 'train_loc',0, x, y, 'position', [x+400 y 100 20], ...
        'OffString', 'end train', ...
		'OnString',  'rand train', ...
		'TooltipString', 'determines if the click train is always at end of cpoke or randomly placed');
	NumeditParam(obj, 'min_delay', .25, x, y, 'position', [x+500 y 100 20], ...
		'TooltipString', 'minimum stim start delay ');
    DispParam(obj,'post_samp',0,x,y, 'position',[x+600 y 100 20],...
        'labelfraction',.5,...
        'TooltipString',sprintf(['post T silence to randomized train loc']));
	next_row(y);
    ...
    ToggleParam(obj, 'ARbias', 0, x, y, 'position', [x y 200 20], ...
        'OffString', 'Not biasing AR probs', ...
        'OnString', 'Biasing AR probs', ...
        'TooltipString', 'Set alternation or repetition probabilities');
    set_callback(ARbias, {mfilename, 'ARbias'});
    NumeditParam(obj, 'repProb', 0.5, x, y, 'position', [x+200 y 150 20], ...
        'labelfraction', 0.4, ...
        'TooltipString', 'Repetition probability, one minus this value is the alternation prob');
    next_row(y);
    NumeditParam(obj, 'R_gammas', 1, x, y, 'position', [x+50 y 200 20], ...
        'labelfraction', 0.3, ...
    ...
    next_row(y);
    NumeditParam(obj, 'L_gammas', -1, x, y, 'position', [x+50 y 200 20], ...
        'labelfraction', 0.3, ...
	...
	NumeditParam(obj, 'easiest', 4, x, y, 'position', [x+200 y 150 20], ...
		'labelfraction', 0.4, ...
		'TooltipString', 'the easiest (endpoints) gamma; this value must be positive');
	NumeditParam(obj, 'hardest', 1, x, y, 'position', [x+350 y 150 20], ...
		'labelfraction', 0.4, ...
		'TooltipString', 'the hardest (closest to 0 midpoint) gamma; this value must be positive');
	NumeditParam(obj, 'N', 4, x, y, 'position', [x+500 y 60 20], ...
		'labelfraction', 0.4, ...
    ...
    next_row(y);
    NumeditParam(obj,'frozen_frac',0,x,y,'position',[x y 150 20],...
        'label','frozen fraction','TooltipString','the fraction of trials that use frozen noise');
    NumeditParam(obj,'n_seeds',10,x,y,'position',[x+150 y 150 20],...
    ...
    NumeditParam(obj, 'base_freq', [450, 1040], x, y, 'position', [x+280 y 150 20], ...
    ...
    NumeditParam(obj, 'ntones', 1, x, y, 'position', [x+430 y 80 20], ...
    ...
    NumeditParam(obj, 'min_ISI', 3, x, y, 'position', [x+300 y 100 20],'labelfraction',0.68, ...
        'TooltipString', 'the minimum time in ms that is allowed between bups','label','min ISI, ms');    
	NumeditParam(obj, 'total_rate', 20, x, y, 'position', [x+400 y 100 20],'labelfraction',0.68, ...
    ...
    ToggleParam(obj, 'vol_on', 1, x, y, 'position', [x+600 y 80 20], ...
    ...
    set_callback({task_type}, {mfilename, 'update_distractor_options'});    
    next_row(y);
    ...
    DispParam(obj, 'is_frozen', 0, x, y, 'position', [x+450 y 100 20], ...
        'labelfraction', 0.8, ...
        'TooltipString', 'whether this trial uses frozen noise');
    DispParam(obj, 'is_fixed', 0, x, y, 'position', [x+550 y 100 20], ...
        'labelfraction', 0.6, ...
        'TooltipString', 'whether this trial uses frozen noise');
    DispParam(obj, 'is_sing', 0, x, y, 'position', [x+650 y 100 20], ...
        'labelfraction', 0.6, ...
        'TooltipString', 'whether this trial uses frozen noise');  
...
%% adjust_volume
  case 'adjust_volume'
    ...
    if is_enabled(headphone_attentuation_dB)
        ...
        snd(1,:) = snd(1,:)/max(snd(1,:)) * 10^-(att(1)/20);S
        snd(2,:) = snd(2,:)/max(snd(2,:)) * 10^-(att(2)/20);
    else
        if vol_on == 0 
            snd = snd*0;
        elseif value(freq_vol) ==1
...
%% check_setup_for_analog_output
  case 'check_setup_for_analog_output'
    if isnan(bSettings('get', 'AO_LINES', 'LASER')) || ...
       isnan(bSettings('get', 'GENERAL', 'CLOCK_SPEED_HZ'))
        is_set_up_for_analog_output.value = 0;
    else
        is_set_up_for_analog_output.value = 1;
    end
%% check_stim_channels
...
%% make_sounds
  case 'make_this_sound'
	srate = SoundManagerSection(obj, 'get_sample_rate');
    if value(freq_vol) == 1
        freq_multiply = value(vol);
    else
        freq_multiply = [1 1];
    end
	% the sound made is at least 1 sec long, or as long as T
    if isempty(value(Bups))
        [snd,lrate,rrate,bpt] = make_pbup_fixed(value(total_rate), value(ThisGamma), srate, value(T), ...
                                           ...
                                           'seed',value(ThisSeed),'task_type',value(task_type),'is_fixed',value(is_fixed),...
                                           'is_sing',value(is_sing),'max_T',value(T_max),'min_T',value(T_min),'freq_multiply',freq_multiply); %#ok<NODEF>;        
    else
        [snd,lrate,rrate,bpt] = make_pbup_fixed(value(total_rate), value(ThisGamma), srate, value(T), ...
                                           'bup_width', value(bup_width), 'first_bup_stereo', value(first_bup_stereo), ...
                                           'crosstalk', value(crosstalk), 'base_freq', value(base_freq), ...
                                           'tones', value(tones), 'bup_ramp', value(bup_ramp),'ntones', value(ntones),...
                                           'avoid_collisions',value(avoid_collisions),'min_ISI',value(min_ISI),...
                                           'seed',value(ThisSeed),'fixed_sound',value(Bups),'task_type',value(task_type),'is_fixed',value(is_fixed),...
                                           'is_sing',value(is_sing),'max_T',value(T_max),'min_T',value(T_min),'freq_multiply',freq_multiply); %#ok<NODEF>;
    end
    if value(fix_nodiffs) && (length(bpt.left) == length(bpt.right))
        while(length(bpt.left) == length(bpt.right))
            c = clock;
    ...
    if value(is_loc_dist) && value(task_type) == 0
        if value(location_dist_side) == 'Left'
            snd(2,:) = zeros(1,numel(snd));
        end
        if value(location_dist_side) == 'Right'
            snd(1,:) = zeros(1,numel(snd));
        end
        if value(location_dist_side) == 'Rand'
            if rand < 0.5
                snd(2,:) = zeros(1,numel(snd));
            else
                snd(1,:) = zeros(1,numel(snd));
    ...
    if value(train_loc)
        if (value(nose_in_center) - value(min_delay)) > value(T)
            rem_T = value(nose_in_center) - value(min_delay) -value(T);
            post_samp.value = ((rem_T).*rand(1));
            post_snd = zeros(2,round(value(post_samp)*value(srate)));
            snd = [snd post_snd];   
        end 
    end
...
%% next_trial
  case 'next_trial'  
      ...
      if ~isempty(violation_history)
          if violation_history(end) ~= 1
	        is_fixed.value = value(fixed_frac)>rand; % reset fixed frac from distractor 
          end
      end
      side = varargin{1};
    ...
    %  setup the distractor type for this trial
    if value(timing_dist)
        is_time_dist.value = value(timing_dist_percent)>rand; 
        if value(is_time_dist) && value(fixed_frac) == 0
            is_fixed.value = 1;
        end
        if value(is_time_dist) && value(fixed_frac) == 1
            is_fixed.value = 0;
        end
    end
    if value(freq_dist)
        is_freq_dist.value = value(freq_dist_percent)>rand;
    end
    if value(location_dist)
        is_loc_dist.value = value(location_dist_percent)>rand;
    end

    %   %determine based on sing fraction whether this trial will be one bup or not
      is_sing.value = value(sing_frac)>rand; 
...
%% update_tones
  case 'update_tones'     
      ...
      if ~value(tone_style)
        if value(is_freq_dist)
            tones.value = value(freq_dist_Hz)* 2.^(0:value(ntones)-1);
        else
...
%% T_resample
  case 'T_resample'
	...
	if rand(1) < p_probe
        ...
        %Since it's a probe duration we should force it to not be a frozen trial
        is_frozen.value = 0;
        c = clock;
        ThisSeed.value = randi(10^9,1) + round(c(6)*1e3);
        try
...
%% update_laser_analog_output
  case 'update_laser_analog_output'
      feval(mfilename, obj, 'check_setup_for_analog_output');
      for param = {'stimulator_style', 'allow_consecutive_stimulation', ...
                   'stimulator_frac','stim_pre', 'stim_freq', ...
                   'stim_pre_jitter','stim_dur','stim_trigger_state',...
                   'ao_is_sine_not_square'}
      ...
      if stim_is_analog_not_digital && ...
         (ao_is_sine_not_square || ~is_set_up_for_analog_output)
          disable(stim_pulse)
      else
          enable(stim_pulse)
      end
  ...
  case 'pick_stimulator'
      ...
      elseif stim_is_analog_not_digital && ~is_set_up_for_analog_output
          ...
          if mystim > 0
            ...
            if value(stim_is_analog_not_digital) && mystim > 0
...
%% mask
  case 'pick_mask'
      ...
      valid_states = {'cpoke1', 'wait_for_spoke'}; % mask not implemented for ITI - AGB 1/8/2020
...
%% update_distractor_options
    case 'update_distractor_options'
        if value(task_type) == 1
            disable(location_dist)
            enable(freq_dist)
        end
        if value(task_type) == 0
            disable(freq_dist)
            enable(location_dist)
        end

...
%
    ...
    pairs = {...
        ...
        'mix_base_freq',    2500; ...
        'mix_freq_frac',    .5;...
        'side_stim',        0;... % if zero, make stereo (TBups task default). If 1, play on left, if 2, play on right
    ...
    if mix_freq
        bup2 = singlebup(srate, 0,'ntones', n_tones, 'width', bup_width*1000, 'basefreq', mix_base_freq(1), 'ramp', bup_ramp);
    end
    ...
    mix_array = zeros(1,length(ici_inds));
    if mix_freq      
        mix_inds = randperm(length(ici_inds),round(length(ici_inds)*mix_freq_frac));
        mix_array(mix_inds) = 1;
    end
    snd = zeros(1,round(T *srate));
    for i =1:length(ici_inds)
        cur_ind = round(ici_inds(i));
        if mix_array(i)
            snd(cur_ind:cur_ind+length(bup)-1) = bup2;
        else

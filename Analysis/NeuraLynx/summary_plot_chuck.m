...
if isempty(sessid)
    ...
    if isempty(renderer); renderer = get(fh,'Renderer'); end
    
	[rh,output]=exampleraster(ref,ts,'ref_label',align_string, ...
		'cnd', condition, ...
...
elseif strcmpi(protocol,'pbups')
    ...
    legend_str = {'L Control','L Laser','R Control','R Laser'};
    rights = pd.sides == 'r';
    laser  = pd.stims == 1;
    condition = (rights*10) + laser;
    
    goright = (pd.sides == 'r' & pd.hits == 1) | (pd.sides == 'l' & pd.hits == 0);
    condition = (goright*10) + laser;
    
    inc_trs = inc_trs & pd.samples >= 1;
...
if noviol_only
        ...
        elseif sum(isnan(pd.hits)) > 0
            noviol = ~isnan(pd.hits);
		else

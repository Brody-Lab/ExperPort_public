function output = WM_assign_rats_to_snugs(force_session,varargin)

if nargin < 1; force_session = 0; end
...
for i=1:numel(RWL)
    ...
    for j=1:numel(RWL{i})
        ...
        if isempty(st) || isempty(ed) 
            ...
            if d == 0
                %start and end times are the same, active water happening
                time_since_water_start = (datenum(datestr(now,'HH:MM:SS')) - datenum(st(k),'HH:MM:SS')) * 24;
                if time_since_water_start > 1
                    %rat has had active water for at least 1 hour.
                    d = time_since_water_start;
                end
            end
    ...
    if isempty(comptemp) || mean(comptemp) < 0.8
        comp(i) = 0;
    end
end
...
if force_session > 0 && force_session < 10
    currsess = force_session;
end

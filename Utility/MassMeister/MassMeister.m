...
try
    handles.RFID = serial(handles.comrfid);
        ...
        if ~isempty(handles.RFID)
            ...
            if ba > 0
                ...
                if sum(strcmp(curr_ratlist,RFID_ratname)) == 1
                    ...
                    %Need to add a wrning here that the scanned rat is not
                    %on the list. Possibly have the system check the all
                    %rats list and go tehre if the rat is on that list, and
                    %throw an error and stop weighing if rat is not on that
                    %list

...
%	end
    ...
    if bSettings('get','RIGS','bpod') == 1
        %apparently we don't have to do this for a bpod rig
    else
  ...
  case 'initialize_machines',
    ...
    if ~isempty(sound_machine_server); sound_machine.value = Initialize(value(sound_machine)); end %#ok<NODEF>
  ...
  % ------------------------------------------------------------------
  %                       GET_TRIALNUM_INDICATOR_FLAG
  % ------------------------------------------------------------------
    
  case 'get_trialnum_indicator_flag',   
    ret = value(trialnum_indicator_flag); %#ok<NODEF>  
  ...
  case 'send_assembler', 
    ...
    if trialnum_indicator_flag==1 && bSettings('get','RIGS','bpod') ~= 1,

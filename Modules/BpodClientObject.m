...
classdef BpodClientObject < handle
    ...
    methods
        ...
        function ok = sendstring(obj, string, varargin)
                                    ...
                                    while i <= obj.nUnreadEvents
                                        ...
                                        if ThisEvent ~= 255
                                            ...
                                            if ThisEvent >= BpodSystem.HW.Pos.Event_Port
                                                if ThisEvent <= numel(BpodSystem.PluginObjects.BcontrolEventCodeMap)
                                                    CEevents(i) = BpodSystem.PluginObjects.BcontrolEventCodeMap(ThisEvent);
                                                else
                                                    %2021-03-03 Hack so if an event comes back that isn't handled we ignore it rather than crashing
                                                    %rigid = bSettings('get','RIGS','Rig_ID');
                                                    %send_text_message('Bpod sendstring FORCING 0 EVENT',['Rig ',num2str(rigid),' CRASH'],'Chuck');
                                        ...
                                        if ThisEvent == 255
                                            if Encountered255 == 0
                                                ...
                                                Encountered255 = 1;
                                                %consider breaking out of while loop here so no events after a 255 can be processed
                                                break
                                            else
            ...
            %catch
                %rigid = bSettings('get','RIGS','Rig_ID');
                %send_text_message('Bpod sendstring UNKNOWN',['Rig ',num2str(rigid),' CRASH'],'Chuck');
                %keyboard
            %end
        end
